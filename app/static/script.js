        ...
        .catch(error => {
            ...
            if (error.response && error.response.data && error.response.data.detail) {
                ...
                if (detail === "Payload too large") {
                    errorMessage = 'The uploaded file is too large. Please choose a smaller file (maximum 10MB).';
                } else if (detail === "Invalid or corrupted image file") {
                    errorMessage = 'The uploaded file is corrupt or invalid. Please choose a correct image file.';
                }
            } else if (error.message) {
                // Fallback to checking error.message if no response detail
                const errorMessages = {
                    'Unsupported image format': 'Please upload only JPEG or PNG images.',
                    'No data received': 'No response received from the server. Please try again.',
                    'Invalid response format': 'Server returned an invalid response format. Please try again.',
                    'Server error': 'A server error occurred. Please try again later.',
                    'Request Entity Too Large': 'The uploaded file is too large. Please choose a smaller file.',
                    'Invalid or corrupted image file': 'The uploaded file is corrupt or invalid. Please choose a correct file.'
                };
